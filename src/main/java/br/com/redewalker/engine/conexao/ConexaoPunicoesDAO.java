...
public class ConexaoPunicoesDAO extends Conexao {
	...
	public void checkTable() {
		...
		try {
			...
			if (!WEngine.get().getConexaoManager().getConexaoAPI().hasColumn(rs, "avaliado")) {
				st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement("ALTER TABLE `"+getNome()+"` ADD `avaliado` tinyint AFTER `ativo`");
				st.executeUpdate();
			}
			if (!WEngine.get().getConexaoManager().getConexaoAPI().hasColumn(rs, "comentario")) {
				st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement("ALTER TABLE `"+getNome()+"` ADD `comentario` text AFTER `ativo`");
				st.executeUpdate();
			}
			if (!WEngine.get().getConexaoManager().getConexaoAPI().hasColumn(rs, "avaliador")) {
				st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement("ALTER TABLE `"+getNome()+"` ADD `avaliador` text AFTER `comentario`");
				st.executeUpdate();
			}
			if (!WEngine.get().getConexaoManager().getConexaoAPI().hasColumn(rs, "despunidor")) {
				st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement("ALTER TABLE `"+getNome()+"` ADD `despunidor` text AFTER `avaliador`");
				st.executeUpdate();
			}
			if (!WEngine.get().getConexaoManager().getConexaoAPI().hasColumn(rs, "avaliacao")) {
				st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement("ALTER TABLE `"+getNome()+"` ADD `avaliacao` tinyint AFTER `despunidor`");
				st.executeUpdate();
			}
	...
	public long createPunicao(boolean ativo, boolean avaliado, boolean avaliacao, String jogador, String avaliador, String despunidor, long data, long ate, String autor, String prova, PunicaoMotivoType motivo, String comentario, PunicoesType tipo) {
		...
		try {
			String[] returnId = { "id" };
			PreparedStatement st = WEngine.get().getConexaoManager().getConexaoAPI().getConexao().prepareStatement(sql, returnId);
			st.setString(1, jogador);
			st.setLong(2, data);
			st.setLong(3, ate);
