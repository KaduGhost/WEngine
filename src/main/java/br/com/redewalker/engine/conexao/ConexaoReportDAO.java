...
public class ConexaoReportDAO extends Conexao {
	...
	public void checkTable() {
		...
		try {
			PreparedStatement st = WEngine.get().getConexaoManager().getConexaoServerAPI().getConexao().prepareStatement(sql);
			st.execute();
		...
		try {
			...
			for (Report punicao : punicoes) {
				...
				if (exists(punicao.getID())) {
					...
					st = WEngine.get().getConexaoManager().getConexaoServerAPI().getConexao().prepareStatement(sql);
