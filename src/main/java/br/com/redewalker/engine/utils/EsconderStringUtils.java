...
public class EsconderStringUtils {
	...
	public static String getStringEscondida(List<String> lore) {
		for (String i : lore) {
			if (EsconderStringUtils.hasStringEscondida(i)) return EsconderStringUtils.extrairStringEscondida(i);
		}
		return null;
	}
	
	public static String esconderString(String hiddenString) {
		return quote(stringToColors(hiddenString));
	}

	public static boolean hasStringEscondida(String input) {
		if (input == null) return false;
		return input.indexOf(SEQUENCE_HEADER) > -1 && input.indexOf(SEQUENCE_FOOTER) > -1;
	}

	public static String extrairStringEscondida(String input) {
		return colorsToString(extract(input));
	}

	public static String replaceStringEscondida(String input, String hiddenString) {
	...
	private static String colorsToString(String colors) {
		...
		if (colors.length() % 2 != 0)colors = colors.substring(0, (colors.length() / 2) * 2);
	...
	private static int hexToUnsignedInt(char c) {
		...
		else throw new IllegalArgumentException("Invalid hex char: out of range");
	}

	private static char unsignedIntToHex(int i) {
		if (i >= 0 && i <= 9) return (char) (i + 48);
		else if (i >= 10 && i <= 15) return (char) (i + 87);
		else throw new IllegalArgumentException("Invalid hex int: out of range");
	}
