...
import br.com.redewalker.engine.sistema.manutencao.ManutencaoType;
...
public class ComandoManutencao extends BukkitCommand {
	...
	public boolean execute(CommandSender sender, String arg1, String[] args) {
		...
		if (WEngine.get().getManutencaoManager().isProcesso() && !args[0].equalsIgnoreCase("cancelar")) {
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("ajuda")) {
				...
				WEngine.get().getManutencaoManager().iniciarManutencao(ManutencaoType.STOP, 30, true);
				MensagensAPI.mensagemSucesso("Servidor irá ser desligado", sender);
				return true;
			}else if (args[0].equalsIgnoreCase("voltar")) {
				if (WEngine.get().getManutencaoManager().isLiberado()) {
					MensagensAPI.mensagemErro("Servidor não está em manutenção", sender);
					return false;
				}
				WEngine.get().getManutencaoManager().getManutencao().reverse();
				MensagensAPI.mensagemSucesso("Servidor saiu do modo de manutenção", sender);
				return true;
			}else if (args[0].equalsIgnoreCase("manutencao")) {
				WEngine.get().getManutencaoManager().iniciarManutencao(ManutencaoType.MANUTENCAO, 0, false);
				MensagensAPI.mensagemSucesso("Servidor entrou em modo de contagem regressiva", sender);
				return true;
			}else if (args[0].equalsIgnoreCase("manuforced")) {
				WEngine.get().getManutencaoManager().iniciarManutencao(ManutencaoType.MANUTENCAO, 30, true);
				MensagensAPI.mensagemSucesso("Servidor está em modo de manutenção", sender);
				return true;
			}
		}else if (args.length == 2) {
			if ((args[0].equalsIgnoreCase("manutencao") || args[0].equalsIgnoreCase("stop")) && !WEngine.get().getManutencaoManager().isLiberado()) {
			...
			if (args[0].equalsIgnoreCase("manutencao")) {
				...
				WEngine.get().getManutencaoManager().iniciarManutencao(ManutencaoType.MANUTENCAO, Integer.parseInt(args[1]), false);
				MensagensAPI.mensagemSucesso("Servidor entrou em modo de contagem regressiva", sender);
				return true;
			}else if (args[0].equalsIgnoreCase("stop")) {
				if (!NumberUtils.isDigits(args[1])) {
				...
				WEngine.get().getManutencaoManager().iniciarManutencao(ManutencaoType.STOP, Integer.parseInt(args[1]), false);
				MensagensAPI.mensagemSucesso("Servidor entrou em modo de contagem regressiva", sender);
				return true;
			}
		}
