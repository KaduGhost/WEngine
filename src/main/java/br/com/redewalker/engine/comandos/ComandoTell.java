...
import br.com.redewalker.engine.sistema.chat.CanalTag;
import br.com.redewalker.engine.sistema.chat.ChatProtecao;
import br.com.redewalker.engine.sistema.chat.ChatUtils;
...
public class ComandoTell extends BukkitCommand {
	...
	public boolean execute(CommandSender sender, String arg1, String[] args) {
		...
		String nick = args[0];
		Usuario j2 = WEngine.get().getUsuarioManager().getUsuario(nick);
	...
	public static void enviarMsg(Usuario sender, String msg) {
		...
		if (!cache.containsKey(sender) || !VanishAPI.UsuarioIsOnline(WEngine.get().getUsuarioManager().getUsuario(p1.getName()), sender)) {
	...
	private static void send(Usuario j1, Usuario j2, String msg) {
		...
		if (le.equals("")) le = "f";
		CanalTag ct = WEngine.get().getChatManager().getChatTag("{grupo}");
		new FancyMessage(corCabeca+"Mensagem de " + WEngine.get().getPlaceholderAPI().checkPlaceholders(ct.getTag(), j1, null) + j1.getNickOriginal() + corCabeca+": ").tooltip("§6Clique para responder.").suggest("/tell " + j1.getNickOriginal() + " ").then(msg).color(ChatColor.getByChar(le)).send(p2);
		new FancyMessage(corCabeca+"Mensagem para " + WEngine.get().getPlaceholderAPI().checkPlaceholders(ct.getTag(), j2, null) + j2.getNickOriginal() + corCabeca+": ").tooltip("§6Clique para responder.").suggest("/tell " + j2.getNickOriginal() + " ").then(msg).color(ChatColor.getByChar(le)).send(p1);
