...
public class InventarioPunirHKicks {
	
	public InventarioPunirHKicks(int pagina, Player p) {
		Usuario j = WEngine.get().getUsuarioManager().getUsuario(p.getName());
		if (!(j instanceof Usuario) || !j.hasPermission("walker.punir")) return;
		List<Punicao> punicoes = WEngine.get().getPunicoesManager().getHistoricoPunicoes(PunicoesType.KICK);
		...
		Inventory inv = Bukkit.createInventory(p, 9*6, "§c§8Historico - Kicks§e");
		for (int i = 0; i < paginas; i++) {
			if (pagina-1 != i) continue;
			int quantItens = punicoes.size()-i*45;
			for (int f = 0; f < 45; f++) {
				if (quantItens == f) break;
				inv.setItem(f, WalkersItens.getItemPunicao("punicaohistorico", p.getName(), punicoes.get(i*45+f)));
			}
		}
