...
import br.com.redewalker.engine.configuracao.ConfigChat;
...
public class ChatManager implements Listener {
	...
	public ChatManager(ConfigChat config) {
		this.config = config;
		file = new File(WEngine.get().getDataFolder(), "chats");
		filetags = new File(WEngine.get().getDataFolder(), "chattags");
	...
	private void salvarCanal(Canal canal) {
		File channel = new File(WEngine.get().getDataFolder(),"chats"+File.separator+canal.getNome().toLowerCase()+".yml");
		if(!channel.exists()) {
		...
		channel2.set("bungee", canal.isBungee());
	...
	public void deleteCanal(Canal c) {
		if(existsCanal(c.getNome())) {
			...
			new File(WEngine.get().getDataFolder(),"chats"+File.separator+c.getNome().toLowerCase()+".yml").delete();
		}
	}
	...
	public void loadCanais() {
		...
		if (config.isLoadDefaultCanais()) {
			...
			criarCanalPermanente(new ChatPermanente("staffbungee", "sb", "{chat} {server}{grupo} {jogador}{chat_cor}: {msg}", true, true, true, false, new Tag("(S)", "§d"), 0, 0, 0, new ArrayList<String>(),true));
			config.setLoadDefaultCanais(false);
		}
		if (file.exists()) {
			for (File channel : file.listFiles()) {
				if(channel.getName().toLowerCase().endsWith(".yml")) {
			        if(!channel.getName().toLowerCase().equals(channel.getName())) channel.renameTo(new File(WEngine.get().getDataFolder(),"chats"+File.separator+channel.getName().toLowerCase()));
	...
	public void loadCanal(File channel) {
		...
		criarCanalPermanente(new ChatPermanente(config.getString("nome"), config.getString("nick"), config.getString("formato"), config.getBoolean("ativo"), config.getBoolean("focus"), config.getBoolean("entreMundos"), config.getBoolean("mostrarMsgCusto"), new Tag(config.getString("tag"), config.getString("cor").replace("&", "§")), config.getInt("custo"), config.getInt("delay"), config.getInt("distancia"), (ArrayList<String>) config.getStringList("mundos"), config.getBoolean("bungee")));
	}
	...
	public void loadChatTags() {
		...
		if (filetags.exists()) {
			for (File channel : filetags.listFiles()) {
				if(channel.getName().toLowerCase().endsWith(".yml")) {
			        if(!channel.getName().toLowerCase().equals(channel.getName())) channel.renameTo(new File(WEngine.get().getDataFolder(),"chattags"+File.separator+channel.getName().toLowerCase()));
	...
	private void salvarTag(CanalTag tag) {
		File channel = new File(WEngine.get().getDataFolder(),"chattags"+File.separator+tag.getNome().toLowerCase().replace("{", "").replace("}", "")+".yml");
	...
	void aoFalar(AsyncPlayerChatEvent e) {
		...
		e.setCancelled(true);
		Canal canal;
		if (!j.getChatFocus().equalsIgnoreCase("default")) canal = WEngine.get().getChatManager().getCanal(j.getChatFocus());
		else canal = WEngine.get().getChatManager().getChatDefault();
		if (!(canal instanceof Canal)) {
			j.setChatFocus("default");
			MensagensAPI.mensagemErro("Nenhum canal disponivel",p);
			return;
		}
		WEngine.get().getChatManager().enviarMensagem(j, msg, canal);
	}
	...
	void aoFalarPorComando(PlayerCommandPreprocessEvent e) {
		...
		Canal canal = WEngine.get().getChatManager().getCanalByNick(comando);
		if (canal != null) {
			e.setCancelled(true);
