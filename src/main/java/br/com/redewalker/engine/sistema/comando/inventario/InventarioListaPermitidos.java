...
public class InventarioListaPermitidos {
	
	public InventarioListaPermitidos(int pagina, Player p) {
		List<ComandoInfos> comandos = WEngine.get().getComandosManager().getAllComandosPermitidos();
		int paginas = 0;
		if (comandos.size()%28 == 0) paginas = comandos.size()/28;
		else paginas = comandos.size()/28+1;
		Inventory inv  = Bukkit.createInventory(p, 9*6, "§8Permitidos - Lista§7");
		...
		inv.setItem(4, WalkersItens.getItemComandos("listapermitidosicon", null));
	...
	private ItemStack getItem(ComandoInfos info) {
		...
		lore.add("");
		lore.add(" §cClique para remover da lista de permitir");
