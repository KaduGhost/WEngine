...
public class ManutencaoManager implements Listener {
	...
	public ManutencaoManager(Config config) {
		this.config = config;
		load();
		new BukkitRunnable(){@Override public void run() {setAberto(true);}}.runTaskLater(WEngine.get(), 20*5);
	...
	private void iniciarCountDown(int tempo) {
		countdown = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(WEngine.get(), new Runnable() {
            long i = tempo;
            public void run() {
	...
	public void setLiberado(boolean set) {
		this.liberado = set;
		WEngine.get().getServerManager().getServer().setManutencao(!set);
	...
	private void load() {
		aberto = travar = false;
		liberado = !WEngine.get().getServerManager().getServer().isManutencao();
