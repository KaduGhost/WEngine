...
import br.com.redewalker.engine.sistema.comando.ComandoInfos;
...
public class InventarioListaComandos {
	...
	public InventarioListaComandos(int pagina, Player p) {
		List<ComandoInfos> comandos = WEngine.get().getComandosManager().getAllComandos();
		int paginas = 0;
		...
		inv.setItem(4, WalkersUtils.WalkersItens.getItemComandos("listacomandosicon", null));
	...
	private ItemStack getItem(ComandoInfos info) {
		...
		lore.add("");
		if (WEngine.get().getComandosManager().containsListaBloqueio(info.getNome())) lore.add(" §cClique para remover da lista de bloqueio");
		else lore.add(" §aClique para adicionar na lista de bloqueio");
		if (WEngine.get().getComandosManager().containsListaPermitir(info.getNome())) lore.add(" §cClique direito para remover da lista de permitir");
