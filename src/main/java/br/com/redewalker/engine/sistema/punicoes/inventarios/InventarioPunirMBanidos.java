...
public class InventarioPunirMBanidos {
	
	public InventarioPunirMBanidos(int pagina, Player p) {
		Usuario j = WEngine.get().getUsuarioManager().getUsuario(p.getName());
		if (!(j instanceof Usuario) || !j.hasPermission("walker.punir")) return;
		List<Punicao> punicoes = WEngine.get().getPunicoesManager().getMeuHistoricoPunicoes(PunicoesType.BAN, j);
		...
		Inventory inv = Bukkit.createInventory(p, 9*6, "§c§8Meus Bans§e");
		for (int i = 0; i < paginas; i++) {
			if (pagina-1 != i) continue;
			int quantItens = punicoes.size()-i*45;
			for (int f = 0; f < 45; f++) {
				if (quantItens == f) break;
				inv.setItem(f, WalkersUtils.WalkersItens.getItemPunicao("punicaomeuhistorico", j.getNickOriginal(), punicoes.get(i*45+f)));
			}
		}
