...
public class GrupoManager {
	...
	public GrupoManager(Config config) {
		this.config = config;
		file = new File(WEngine.get().getDataFolder()+File.separator+"grupos"+File.separator, "data.yml");
	...
	public void addPermission(GrupoType tipo, String permission) {
		WEngine.get().getGruposManager().getGrupo(tipo).addPermission(permission);
	}
	
	public void removePermission(GrupoType tipo, String permission) {
		WEngine.get().getGruposManager().getGrupo(tipo).removePermission(permission);
	}
	
	public Tag getTagGrupo(GrupoType tipo) {
		return WEngine.get().getGruposManager().getGrupo(tipo).getTag();
	}
	
	public String getNomeGrupo(GrupoType tipo) {
		return WEngine.get().getGruposManager().getGrupo(tipo).getNome();
	}
	...
	private void checkData() {
		WEngine.get().getConexaoManager().getGrupoConnection().checkTable();
		...
		for (GrupoType grupo : EnumSet.allOf(GrupoType.class)) {
			...
			WEngine.get().getConexaoManager().getGrupoConnection().createGrupo(grupo.toString(), grupo.toString(), "semtag", "semcor", "nenhuma");
		}
		saveConfig();
	}
