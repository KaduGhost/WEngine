package br.com.redewalker.engine.sistema.comando;

import br.com.redewalker.engine.apis.MensagensAPI;
import br.com.redewalker.engine.sistema.comando.inventario.*;
import br.com.redewalker.engine.sistema.usuario.Usuario;
import br.com.redewalker.engine.WEngine;
import br.com.redewalker.engine.configuracao.ConfigComando;
import br.com.redewalker.kaduzin.engine.sistema.comando.inventario.*;
...
public class ComandosManager implements Listener {
	...
	public ComandosManager(ConfigComando config) {
		...
		file = new File(WEngine.get().getDataFolder()+File.separator+"comandos"+File.separator, "data.yml");
	...
	public void adicionarComando(Command comando, int delay, String permission, boolean registrado, boolean ativo) {
		...
		((CraftServer) WEngine.get().getServer()).getCommandMap().register("walker", comando);
	}
	...
	private void load() {
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				Iterator<Command> a = ((CraftServer) WEngine.get().getServer()).getCommandMap().getCommands().iterator();
				while (a.hasNext()) {
					Command i = a.next();
		...
		}.runTaskLaterAsynchronously(WEngine.get(), 0);
	}
	...
	private void loadTabComplete() {
		...
		manager.addPacketListener(new PacketAdapter(WEngine.get(), new PacketType[] { PacketType.Play.Client.TAB_COMPLETE }) {
			public void onPacketReceiving(PacketEvent event) {
				...
				Usuario j = WEngine.get().getUsuarioManager().getUsuario(event.getPlayer().getName());
