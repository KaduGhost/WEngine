...
public class InventarioVIPsHistorico {
	
	public InventarioVIPsHistorico(int pagina, Player p, Servers server, InventarioVipsType tipo) {
		...
		ArrayList<VIP> vips = WEngine.get().getVipManager().getVIPs(server);
		Collections.sort(vips, Collections.reverseOrder());
		int paginas = 0;
		if (vips.size()%45 == 0) paginas = vips.size()/45;
		else paginas = vips.size()/45+1;
		Inventory inv = Bukkit.createInventory(p, 9*6, "§a§8VIPs - "+server.toString());
