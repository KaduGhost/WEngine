...
public class InventarioPunirMMutes {
	
	public InventarioPunirMMutes(int pagina, Player p) {
		Usuario j = WEngine.get().getUsuarioManager().getUsuario(p.getName());
		if (!(j instanceof Usuario) || !j.hasPermission("walker.punir")) return;
		List<Punicao> punicoes = WEngine.get().getPunicoesManager().getMeuHistoricoPunicoes(PunicoesType.MUTE, j);
